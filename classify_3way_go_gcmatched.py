...
immune = load_fasta("go_immune_gcmatched.fasta")
gpcr   = load_fasta("go_gpcr_gcmatched.fasta")
trans  = load_fasta("go_transmembrane_gcmatched.fasta")
print(f"Immune: {len(immune)}, GPCR: {len(gpcr)}, Transmembrane: {len(trans)}")

sequences = immune + gpcr + trans
labels = [0]*len(immune) + [1]*len(gpcr) + [2]*len(trans)
...
names = ["immune", "gpcr", "transmembrane"]
print(f"\n=== RESULTS seed={seed} (GO 3-way) ===")
...
for i, n in enumerate(names):
    print(f"{n[:12]:>12} " + "  ".join(f"{v:>6}" for v in cm[i]))
