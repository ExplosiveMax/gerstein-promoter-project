...
uorf   = load_fasta("uorfs_capped_trimmed.fasta")
within = load_fasta("uorf_negatives_chr22.fasta")
nonutr = load_fasta("nonutr_4way.fasta")
print(f"uORF(trimmed): {len(uorf)}, withinUTR: {len(within)}, nonUTR: {len(nonutr)}")

sequences = uorf + within + nonutr
labels = [0]*len(uorf) + [1]*len(within) + [2]*len(nonutr)
...
names = ["uORF", "withinUTR", "nonUTR"]
print(f"\n=== RESULTS seed={seed} (3-way, no random, trimmed uORF) ===")
...
for i, n in enumerate(names):
    print(f"{n[:10]:>10} " + "  ".join(f"{v:>9}" for v in cm[i]))
