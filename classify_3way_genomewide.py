...
# Forced CPU: a background job on this same machine is using MPS (unified
# memory on Apple Silicon is shared between CPU/GPU), so this run
# deliberately avoids touching MPS to not contend with / destabilize it.
...
functional = load_fasta("functional_promoters_genomewide.fasta")
pseudo     = load_fasta("pseudo_promoters_genomewide.fasta")
random_seq = load_fasta("random_genomewide.fasta")
print(f"Functional: {len(functional)}, Pseudo: {len(pseudo)}, Random: {len(random_seq)}")

# Labels: 0 = functional promoter, 1 = pseudogene promoter, 2 = random
sequences = functional + pseudo + random_seq
labels = [0]*len(functional) + [1]*len(pseudo) + [2]*len(random_seq)
...
names = ["functional", "pseudogene", "random"]
print(f"\n=== RESULTS seed={seed} (genome-wide) ===")
